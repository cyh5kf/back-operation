...
export default class TalkerApplication extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onClickRecordOperation(operationType, record) {
        ...
        var request = (isReject) ? AjaxUtil.TalkerApplicationReject : AjaxUtil.TalkerApplicationApprove;
    ...
    queryTableContent(callback) {
        var queryCondition = this.state.queryCondition;
        var that = this;
        that.setState({loading: true});

        AjaxUtil.queryTalkerApplicationList(queryCondition, function (d) {
            var applicationMoList = d.rows || [];
            applicationMoList.forEach(function (m, i) {
                m.key = i;
                m.birthday = m.birth_year+'-'+m.birth_month+'-'+m.birth_day;
            });
            that.setState({
                loading: false,
                        ...
                        <Row className="r3" type="flex" justify="start">
                            <Col span="24" className="tableWrapper">
                                <Table columns={columns} dataSource={tableRows} pagination={pagination}
                                       locale={i18n.tableLocale} scroll={{ x: 1000 }} ></Table>
                            </Col>
                        </Row>
                    </div>
                </Spin>
